...
export class ClubScene extends Phaser.Scene {
  ...
  _renderTurn(){
    ...
    if (this.debug.text){
      this.debug.text.setText(`aff=${this.affinity} int=${this.interest} irr=${this.irritation}`);
    }
  }
  ...
  async _submitText(text){
    ...
    try{
      ...
      this.pending = false;
      if (!this.ended) this._setFixedBarEnabled(true);
    }
  }
